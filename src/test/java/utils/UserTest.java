...
class UserTest {
	...
	void setUp() throws Exception {
		...
		TestEvent1 = new Event("001", "Evento1", new Date(), new Date(), "Evento1 descripcion", "admin");
		TestEvent2 = new Event("002", "Evento2", new Date(), new Date(), "Evento2 descripcion", "user2");
	}
	...
	void removeEventTest() {
		TestUser.putEvent(TestEvent1);
		TestUser.putEvent(TestEvent2);
		TestUser.removeEvent(TestEvent1.getId());
	...
	void correctUserDataTest() {
		...
		() -> assertTrue(TestUser.isCorrect(), "Comprobaci�n de usuario correcto fallida"),
		() -> assertFalse(TestUser2.isCorrect(), "Comprobaci�n de usuario incorrecto fallida") 
				);
	}
