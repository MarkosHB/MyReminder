...
class DatabaseTest {
	...
	@Test
	void addUserTest() {
		db.addUser(TestUser5.getName(), TestUser5);
		assertAll("users add",
		() -> assertTrue(db.containsUserName("user1") , "Usuario no encontrado"),
		() -> assertTrue(db.containsUserName("user5"), "No se añadió el usuario correctamente"),
	...
	void addMailTest() {
		...
		() -> assertTrue(db.containsMail(TestUser5.getMail()), "No se añadió el correo correctamente"),
